...
final case class BackupOptions(
  ...
  def toArgs : Chunk[String] =
    ...
    if(host.isDefined) then b += s"--host=${host.get}" else ()
    ...
    if(parent.isDefined) then b += s"--parent=${parent.get}" else ()
    ...
    if(stdinFilename.isDefined) then b += s"--stdin-filename=${stdinFilename.get}" else ()
    if(stdinFromCommand) then b += "--stdin-from-command" else ()
    if(tag.nonEmpty) then b ++= tag.map(e => s"--tag=$e") else ()
    if(time.isDefined) then b += s"--time=${time.get}" else ()
